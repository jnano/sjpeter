...
export default function AdminNoticesPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const url = editId ? `${API}/api/notices/${editId}` : `${API}/api/notices/`;
    ...
    try {
      ...
      if (editId) {
        setNotices((prev) => prev.map((n) => n.id === editId ? data : n));
      } else {
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-8 p-6 bg-gray-50 border border-gray-200 rounded-xl space-y-4">
          ...
          <div className="flex gap-2 justify-end">
            <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-sm hover:bg-gray-100">취소</button>
      ...
      <div className="space-y-2">
        {notices.length === 0 && <p className="text-center py-12 text-gray-500">등록된 공지가 없습니다.</p>}
        {notices.map((n) => (
          <div key={n.id} className="p-4 bg-white border border-gray-200 rounded-xl">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => openEdit(n)} className="px-3 py-1.5 text-xs border border-gray-300 rounded-lg hover:bg-gray-50">수정</button>
                <button onClick={() => handleDelete(n.id)} className="px-3 py-1.5 text-xs border border-red-200 text-red-500 rounded-lg hover:bg-red-50">삭제</button>
              </div>
            </div>
          </div>
        ))}

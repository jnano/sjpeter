...
def create_route(body: TransportRouteIn, db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
...
def update_route(route_id: int, body: TransportRouteUpdate, db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
...
def delete_route(route_id: int, db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    row = _get_or_404(route_id, db)
    db.delete(row)
    db.commit()
...
def reorder_routes(body: ReorderBody, db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
